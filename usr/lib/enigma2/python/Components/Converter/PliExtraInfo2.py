...
caid_data = (
  ...
  ( "0x900",  "0x9ff", "NDS",            "NDS", False  ),
  ( "0xb00",  "0xbff", "Conax",          "CO", False  ),
  ( "0xd00",  "0xdff", "CryptoWorks",    "CW", False  ),
  ...
  ("0x4ae0", "0x4ae1", "DreCrypt",       "D",  False ),
  ("0x4aee", "0x4aee", "BulCrypt",       "BUL", False ),
  ("0x5581", "0x5581", "BulCrypt",       "BUL", False )
)
...
class PliExtraInfo2(Poll, Converter, object):
  def __init__(self, type):
    ...
    self.ca_table = (
      ...
      ("CryptoCaidNDSAvailable",	"NDS",	False),
      ...
      ("CryptoCaidBulCrypt1Available","BUL",	False),
      ("CryptoCaidBulCrypt2Available","BUL",	False),
      ...
      ("CryptoCaidNDSSelected",	"NDS",	True),
      ...
      ("CryptoCaidBulCrypt1Selected",	"BUL",	True),
      ("CryptoCaidBulCrypt2Selected",	"BUL",	True),
  ...
  def createCryptoBar(self, info):
    ...
    for caid_entry in caid_data:
      ...
      if color != "\cFF7?7?7?" or caid_entry[4]:
        if res: res += "  "
  ...
  def createChannelNumber(self, fedata, feraw):
    return "DVB-T" in fedata.get("system") and fedata.get("channel") or ""

  def createSymbolRate(self, fedata, feraw):
    if "DVB-T" == fedata.get("system"):
  ...
  def getText(self):
    ...
    if not info:
      return ""
    if self.type == "CryptoText":
      self.getCryptoInfo(info)
      caid = "%0.4X" % int(self.current_caid,16)
      caidsystemtext = self.systemTxtCaids.get(caid[:2])
      if caidsystemtext == None:
        return "FREE TO AIR"
      return "%s" % caidsystemtext
    ...
    if not feraw:
      ...
      fedata = feraw and ConvertToHumanReadable(feraw)
    else:
      fedata = self.fedata
  ...
  def getBool(self):
    ...
    if self.type == "TunerSystemIsSat":
      if self.createTunerSystem(fedata) == "DVB-S" or self.createTunerSystem(fedata) == "DVB-S2":
        return True
      else:
        return False
    if self.type == "IsMPEG2":
      if self.createVideoCodec(info) == "MPEG2":
        return True
      else:
        return False
    if self.type == "IsH264":
      if self.createVideoCodec(info) == "AVC":
        return True
      else:
        return False
        
    if self.type == "IsH265":
      if self.createVideoCodec(info) == "HEVC":
        return True
      else:
        return False
        
    if self.type == "IsAudioStereo":
      audio = service.audioTracks()
      idx = audio.getCurrentTrack()
      if audio:
        i = audio.getTrackInfo(idx)
        description = i.getDescription()
        if description not in ("AC3", "AC3+", "DTS", "DTS-HD", "AC-3"):
          return True
      return False
    if self.type == "IsAudio51":
      ...
      return False
    if self.type == "Is576":
      return info.getInfo(iServiceInformation.sVideoHeight) > 480 and info.getInfo(iServiceInformation.sVideoHeight) < 720
    if self.type == "Is480":
      return info.getInfo(iServiceInformation.sVideoHeight) <= 480 and info.getInfo(iServiceInformation.sVideoHeight) > 0
    request_caid = None
